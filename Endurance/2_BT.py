...
class BtEndurance(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if len(sys.argv)>1:       
            serino = sys.argv[1] 
        cls.mod = Bt(serino, "Set_BTConnect")
        cls.sb = StatusBar(cls.mod.device, "sb_BtConnect")
        cls.mod_cfg = GetConfigs("BtConnect")
    ...
    def testBtINSetting(self):
        test_times = int(self.mod.dicttesttimes.get("Bt_in_set".lower()))
        self.mod.enter()
    ...
    def testBtINStatusBar(self):
        test_times = int(self.mod.dicttesttimes.get("Bt_in_status".lower()))
        if self.sb.drag_down():
            for i in range(test_times):
                if self.sb.switch_Bt():
